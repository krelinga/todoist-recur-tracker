...
- **The completion-events fetch window is bounded by an in-memory scan cursor, not by tracked rows' own cursors.** Naively using the oldest `last_completion_at` across all tracked rows as the fetch's lower bound would mean a single infrequently-recurring task (an annual reminder, say) forces an ever-widening account-wide scan on *every* cycle, forever. That's unnecessary: the fetch only needs to include events since the last time it looked, because each row's own `last_completion_at` already decides independently whether a given event is new *to that row*, regardless of how wide the fetch was. So an in-memory (never persisted) cursor tracks the last successful fetch time instead, and each cycle's `dateFrom` is that cursor minus a fixed 2-day fudge factor. The fudge factor covers a narrower failure mode than a full outage: if one specific row's tracking check keeps failing for a while (an unrelated per-task network blip) without failing the fetch itself, that row won't get a chance to consume an already-fetched event that cycle — the 2-day margin gives it room to recover before that event ages out of the window. Being in-memory rather than persisted is deliberate: it resets on every restart, so the very first fetch after any restart or crash always falls back to the thorough per-row-minimum bootstrap described above (guaranteeing no gap), and only later cycles use the cheap cursor-minus-fudge window. The cursor only advances on a *successful* fetch, so a run of failed fetches keeps widening the window on its own without needing the fudge factor to do that job too. Because the margin is generous relative to normal operation, using the local clock for this cursor — unlike the per-row cursor in section 4, which deliberately avoids the local clock — is safe.
...
| In-memory scan cursor with a 2-day fudge factor (vs. always fetching from the oldest tracked row's own cursor) | Keeps the completion-events fetch window small and roughly constant-cost regardless of how rarely any individual tracked task recurs — an annually-recurring task no longer forces an account-wide year-long rescan on every cycle; only the one-time bootstrap fetch right after a restart does that (see section 6) | Not persisted, so every restart re-triggers one wider bootstrap fetch (cheap, one-time, not a correctness issue). The 2-day fudge is a judgment call, not a proof: it bounds a narrower risk than a full outage (which the not-advance-cursor-on-failure rule already handles correctly regardless of fudge size) — specifically, one row's tracking check failing for longer than 2 days straight while the fetch itself keeps succeeding could in principle let that row miss a completion. Considered acceptable given how much rarer that specific failure mode is than an ordinary outage |
